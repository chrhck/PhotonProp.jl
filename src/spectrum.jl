...
export Spectrum, Monochromatic, CherenkovSpectralDist, Cherenkov
...
step size `step_size`.
"""
function frank_tamm_inverted_cdf(wl_range::Tuple{T, T}, medium::MediumProperties{T}, step_size::T = T(1)) where T
...
struct Cherenkov{T <: Real, N} <: Spectrum
    wl_range::Tuple{T, T}
    knots::SVector{N, T}

    Cherenkov(::Tuple{T, T}, ::SVector{N, T}) where {T <: Real, N <: Integer} = error("default constructor disabled")

    function Cherenkov(wl_range::Tuple{T, T}, interp_steps::U, medium::V) where {T <: Real, U <:Integer, V <: MediumProperties{T}}
